...
{   
     ...
     * @return [] info 新闻首页数据
     */
    public function actionList($type_id = 0, $simple = '0', $only_infomaion = '0', $only_hot = '0', $page = 1, $page_size = 15)
    {
        $search = News::search();
        ...
        $items = array_map(function ($d) use ($simple){
            $item = [
                'id' => $d->id,
            ...
            if ($simple !== '1') {
                $item['short_content'] = \common\cms\helper\Content::subString(strip_tags($d->content), 200);
            }
            $item['hits'] = $d->hits;
